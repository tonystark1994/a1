...
var require_hash = __commonJS({
  "src/hash.js"(exports2, module2) {
    var crypto2 = require("crypto");
    var hashv12 = {
      hash: (uid2, did2, seed, date, salt) => crypto2.createHash("sha256").update(`${uid2}/${did2}/${seed}/${date}/${salt}`).digest("hex"),
      count: (hash) => {
        let count = 0;
    ...
    var hashv32 = {
      hash: (uid2, did2, seed, date, salt) => crypto2.createHash("sha256").update(`${uid2}/${did2}/${seed}/${date}/${salt}`).digest(),
      count: (hash, current_difficulty) => {
        let count = 0;
        for (let hashIter = 0; hashIter < hash.length; ++hashIter) {
          if ((hash.length - hashIter) * 2 < current_difficulty - count) {
...
var { hashv3, hashv1, hashv2 } = require_hash();
var fetch = globalThis.fetch || require_lib2();
var performance = globalThis.performance || require("perf_hooks").performance;
var uid = "375707065";
var hashFinderVariant = 1;
var did = `${Math.ceil(Math.random() * 1e12)}`;
...
(async () => {
  ...
  const hashFinderAlgs = [
    [
      () => hashv1.hash(uid, did, lastBlock.seed, lastBlock.date, salt),
      () => hashv1.count(hash)
    ],
    [
  ...
  function setLastBlock(block) {
    ...
    switch (lastBlock.current_difficulty) {
      case 20:
        checkLastBlockTiming = 5e3;
        break;
      case 19:
        checkLastBlockTiming = 4e3;
        break;
      case 18:
        checkLastBlockTiming = 3e3;
        break;
      default:
        checkLastBlockTiming = 1e4;
        break;
    }
  }
  function testHashrate() {
    ...
    req(`/testHashrate?uid=${uid}&did=${did}`).then((testHashRate) => {
      ...
      if (testHashRate.startsWith("{"))
        testHashRate = JSON.parse(testHashRate);
      else
        return;
      let salt2;
      for (salt2 = 0, hash2; ; salt2++) {
        hash2 = crypto.createHash("sha256").update(`${uid}/${testHashRate.seed}/${testHashRate.date}/${salt2}`).digest("hex");
        if (hash2 === testHashRate.hash) {
          break;
        }
      }
      hasAsyncOperation = true;
      req(`/verifyHashrate?uid=${uid}&did=${did}&salt=${salt2}`).then((hashrate) => {
        hasAsyncOperation = false;
      });
    });
  }
  const checkLastBlockSync = () => {
    hasAsyncOperation = true;
    fetch(`https://ykc_drop.nk-systems.ru/lastBlock?uid=${uid}&did=${did}`).then((res) => res.json()).then((newLastBlock) => {
      hasAsyncOperation = false;
